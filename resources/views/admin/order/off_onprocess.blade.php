    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-header d-flex">
                <div class="table-head text-left"><i class="fas fa-table me-1"></i>On Process Order List</div>
                                   ...
                                   <td>@if(isset($order->delivery_date)){{$order->delivery_date}} @endif ,@if(isset($order->deliveryTime->time)) {{$order->deliveryTime->time}} @endif</td>
                                    ...
                                    <td>
                                        @if(Auth::user()->action_process == 1)
                                            @if ($order->status == 'off_pro')
                                            <a href="{{route('order.process',$order->id)}}" id="procesing" onclick="processing({{$order->id}})" class="btn btn-edit procesing" data-bs-toggle="modal" data-bs-target="#myModal">Offer Process</a>
                                            @endif
                                        @endif
                                    </td>
                                    <td class="text-center text-nowrapt" >
                                            ...
                                            @if(Auth::user()->action_delete == 1)
                                                <button  type="submit" class="btn btn-delete" title="Cancel" onclick="return confirm('Are you sure you want to cancel this order?');"><i class="fas fa-window-close"></i></button>
                                            @endif
                                        </form>
