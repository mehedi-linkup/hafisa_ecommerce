                                   ...
                                   <td>@if(isset($order->delivery_date)){{$order->delivery_date}} @endif ,@if(isset($order->deliveryTime->time)) {{$order->deliveryTime->time}} @endif</td>
                                    ...
                                    <td>
                                        @if(Auth::user()->action_process == 1)
                                            @if ($order->status == 'on')
                                            <a href="{{route('order.process',$order->id)}}" id="procesing" onclick="processing({{$order->id}})" class="btn btn-edit procesing" data-bs-toggle="modal" data-bs-target="#myModal">Process</a>
                                            @endif
                                        @endif
                                    </td>
